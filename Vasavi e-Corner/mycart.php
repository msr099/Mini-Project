...
if (isset($_REQUEST['cid'])) {
		$cid = $_REQUEST['cid'];
		if(mysqli_query($link,"DELETE FROM orders WHERE pid='$cid' AND uid='$user'")){
	...
	try {
		...
		if(empty($_POST['address'])) {
			throw new Exception('Address can not be empty');
						...
						$result = mysqli_query($link,"SELECT * FROM cart WHERE uid='$user'");
						...
						while ($get_p = mysqli_fetch_assoc($result)) {
							...
							mysqli_query($link,"INSERT INTO orders (uid,pid,quantity,oplace,mobile,odate) VALUES ('$user','$pid',$num,'$_POST[address]','$_POST[mobile]','$d')");
						}

						if(mysqli_query($link,"DELETE FROM cart WHERE uid='$user'")){
...
<body style="background-color:#ddd;">
	...
	<div style="margin-top: 20px; padding: 0 10%;">
		<div style=" margin: 0 auto; width: 60%;float: left;">
		...
		$result = mysqli_query($link,"SELECT * FROM user WHERE id='$user'");
			$get_user_email = mysqli_fetch_assoc($result);
				...
				echo '<h3 style="color:black;font-size:25px;"> Delivery Location: </h3>';
							...
							<div class="signupform_text"></div>
								...
								<form action="" method="POST" class="registration">
									...
									<div class="signup_form" >
										...
										<div>
											<input name="order" style="color:#1d3557;" class="uisignupbutton signupbutton" type="submit" value="Confirm Order">
										</div>
